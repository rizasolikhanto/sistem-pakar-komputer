...
@section('content')
    ...
    <div class="card">
        <div class="d-flex align-items-center pl-4 pt-4 flex-row justify-content-between flex-wrap">
            <div>
                <h5 class="bg-white">Daftar Kerusakan Komputer</h5>
                <a class="btn btn-success mt-3" href="{{ route('admin.kerusakan.create') }}">
        ...
        <div class="card-body">
            <div class="table-responsive text-nowrap">
                <table class="table table-hover table-bordered table-striped">
                    ...
                    @foreach($kerusakan as $g)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $g->kode_kerusakan }}</td>
                            <td>{{ $g->kerusakan }}</td>
                            <td class="d-flex ">
                                <a href="{{ route('admin.kerusakan.edit', $g->id) }}" class="btn btn-sm btn-primary mx-1">
                                    <i class="fas fa-edit"></i>Ubah
                                </a>
                                <form action="{{ route('admin.kerusakan.destroy', $g->id) }}" method="POST" enctype="multipart/form-data" onsubmit="return confirm('Konfirmasi Hapus Data . !!')">
                                    @csrf
                                    @method('delete')
